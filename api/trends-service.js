...
function extractAmazonMoversProducts(
  html,
  amazonConfig
) {
  const products = [];
  ...
  const productLinkPattern =
    /href=["']([^"']*(?:\/dp\/|\/gp\/product\/)([A-Z0-9]{10})[^"']*)["']/gi;

  const linkMatches = [
    ...htmlText.matchAll(productLinkPattern)
  ];
  ...
  for (const linkMarker of linkMatches) {
    const relativeLink =
      linkMarker[1];
    ...
    const imageAltMatch =
      productImageTag.match(
        /\balt=["']([^"']+)["']/i
      );

    const anchorTitleMatch =
      productBlock.match(
        new RegExp(
          `<a\\b[^>]*href=["'][^"']*(?:\\/dp\\/|\\/gp\\/product\\/)${asin}[^"']*["'][^>]*[\\s\\S]*?<span\\b[^>]*>([\\s\\S]{8,600}?)<\\/span>`,
          "i"
        )
      );
